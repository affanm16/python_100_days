...
=>Function caching can have a significant impact on the performance of a program, particularly for computationally
 expensive functions.
...
function so that you can reuse the results instead of recomputing them every time the function is called. 
In Python 3, function caching can be achieved using the functools.lru_cache decorator, which provides an easy 
and efficient way to cache the results of a function. 
Whether you're writing a computationally expensive program, or just want to simplify your code, function caching 
is a great technique to have in your toolbox.
"""
import functools
import time
@functools.lru_cache(maxsize=None)
...
print("done for 2")
print(fx(10))
print("done for 10")#it will execute fo every 5 sec

#it will not take time for all these calls...it will diplay from the catch
print(fx(20))
print("done for 20")
print(fx(2))
print("done for 2")
