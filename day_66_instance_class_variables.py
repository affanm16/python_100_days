...
class Employee:
    ...
    def __init__(self,name):#constructor
        self.name=name
        self.raise_amount=0.002#instance variable

    def showDetails(self):#method
        print(f"the name of the employee is {self.name} and raise amount in {self.company_Name} is {self.raise_amount}")
e1=Employee("affan")
e1.raise_amount=0.3
e1.company_Name="Apple India"#it changes for e1
e1.showDetails()
e2=Employee("john")
e2.showDetails()
...
=>Instance variables are defined at the instance level and are unique to each instance of the class. 
 They are defined inside the init method and are usually used to store information that is specific to each 
...
=>In summary, class variables are shared among all instances of a class and are used to store information that
 is common to all instances. Instance variables are unique to each instance of a class and are used to store
  information that is specific to each instance. Understanding the difference between class variables and instance
   variables is crucial for writing efficient and maintainable code in Python.

=>It's also worth noting that, in python, class variables are defined outside any methods and don't need to be
 explicitly declared as class variable. They are defined in the class level and can be accessed via classname.
 varibale_name or self.class.variable_name. But instance variables are defined inside the methods and need to be
